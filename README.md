...
所以有一道熔断：思考字符超过 `RSI_MAX_REASONING_CHARS`（默认 300000）而答案还不足 200 字符时，
立刻中止并说明原因。这个阈值是**兜底，不是目标**——同一个模型成功那次思考了 115,419 字符才写出
27× 的 solver，所以卡太紧会误杀好结果（120k 的上限就让一次 CI 在 120,018 处功亏一篑）。这是省钱，也是给出诊断——它区分了"模型在慢慢想"和"模型不会做这个任务"。
（熔断只在正文尚未开始时开火，所以一个"想得久但终会作答"的模型不会被误杀。）

**注意 `<think>` 标签。** 有些模型/代理把思考过程塞在 `delta.content` 里用 `<think>…</think>` 包着，
而不只是放在 `reasoning_content` 字段。第一版熔断因此形同虚设——它测的"正文长度"里装的全是思考内容。
现在流式解析会增量剥离这些块，熔断测的是**剥离后的真实答案长度**，块解析器也永远看不到 `<think>` 正文。
